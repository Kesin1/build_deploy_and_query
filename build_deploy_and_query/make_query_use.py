...
def make_query_use(data_kb_with_vectors: list, embed, args, logger):
    """Make a query with the use model

    Parameters
    ----------
    data_kb_with_vectors: list
        list with entries of kb with vectors (look build_use.py, adding_vectors)
    ...
    use_scores = sorted(
        use_scores, key=operator.itemgetter("score"), reverse=True)
